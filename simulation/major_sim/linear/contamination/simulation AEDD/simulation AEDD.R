...
foreach(index = 1:dim(combo)[1])%dopar%{
  ...
  for(i in start_row:1000){
    ...
    for(attack in c("Omniscient","Gaussian")){#"Omniscient","Gaussian",
      if(attack=="Omniscient"){theta_list[1:floor(K^(1/4)),]<--10^10}
      if(attack=="Gaussian"){theta_list[1:floor(K^(1/4)),]<-rnorm(floor(K^(1/4)),0,200^2)}
      if(attack=="Bit-flip"){theta_list[1:floor(K^(1/4)),]<-theta_list[1:floor(K^(1/4)),]*(-1)}
      RAEDD_estimate<-nleqslv(c(2,1),robust_aggregation,theta_vector=theta_list,n_vector=rep(n,K),sigma=full_sigma*n,c=3000,
                              control=list(allowSingular=TRUE))$x
      RAEDD_SE<-sqrt(diag(full_sigma)/K)
      ...
      fwrite(estimation,paste0(location,"//K=",K,"_n=",n,"_AEDD_",attack,".csv"),append=TRUE,col.names=FALSE)
    }
    ################################################################
  }
}
