...
foreach(index = 1:dim(combo)[1])%dopar%{
  ...
  for(i in start_row:1000){
    ...
    for(k in 1:K){
      X<-rnorm(n*2)
      dim(X)<-c(n,2)
      prob<-exp(X%*%theta)/(1+exp(X%*%theta))
    ...
    for(k in 1:K){
      ...
      model_sub<-glm(Y ~.-1,family=binomial(link='logit'),data=subsample)
    ...
    fwrite(estimation,paste0(location,"//K=",K,"_n=",n,"_AEDD",".csv"),append=TRUE,col.names=FALSE)
    ################################################################
  }
}
