...
#"Uncontaminated", "Covariate", or "Response"
attack <- "Covariate"
...
#this is for logistic regression only
local_sigma_estimator <- function(theta,X,Y){
  
  local_n <- dim(X)[1]
  
  linear_predictor <- as.numeric(X%*%theta)
  probability <- plogis(linear_predictor)
  
  #keep probabilities away from exactly 0 and 1
  #this prevents a singular Hessian under extreme contamination
  probability[probability<10^(-8)] <- 10^(-8)
  probability[probability>1-10^(-8)] <- 1-10^(-8)
...
for(i in 1:R){
  ...
  for(k in 1:K){
    ...
    #independently flip each binary response
    #with probability 0.2 on each attacked machine
  ...
  estimation <- data.frame(t(data.frame(c(as.vector(cbind(robust_estimate,robust_SE,CI)),
          hit_rate))))
